...
DEFAULT_VOLUME_FOLDER = 'Unix/Linux/OpenStack/Volumes'
DEFAULT_SERVER_FOLDER = 'Unix/Linux/OpenStack/Servers'
...
class StorageCenterApi(object):
    ...
    def list_replays(self, volume):
        """Gets all replays for a given volume
        :returns: All replays as a list
        """
        LOG.debug('Getting list of all replays for volume %s.' % self._get_id(volume))
        result = []

        r = self.client.get('StorageCenter/ScVolume/%s/ReplayList' % self._get_id(volume))
        if self._check_result(r):
            retval = self._get_json(r)
            if isinstance(retval, list):
                result = retval
    ...
    def create_view(self, name, replay, volume_folder, notes = ""):
        """Restore a replay as a volume on the Storage Center.
        It will create it in a folder given in volume_folder param.
        :param name: The name of the new volume
        :param replay: the replay to restore from
        :param volume_folder: The folder to restore the volume into.
        :param notes: Notes string which will be put in this volume
        ...
        if not folder:
            LOG.warn("Folder unable to be found. Will create in root")
        ...
        r = self.client.post('StorageCenter/ScReplay/%s/CreateView' % self._get_id(replay),
                             payload)
        if self._check_result(r):
            ...
            raise Exception('ScVolume creation failed. {}'.format(self._get_json(r)))
